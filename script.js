...
const generateTemplate = todo => { //defining the generateTemplate function
...
addForm.addEventListener('submit', e =>{   //attaching a submit event to the form field "add new"
...
if(todo.length){ //if the inserted value has only spaces don`t generate template
...
//delete added stuff
list.addEventListener('click', e =>{ //adding event listener set to action "click" to the list selected at line 3
if(e.target.classList.contains('delete')){ //using "target" method to target only the elements with the class 'delete' in the list
    e.target.parentElement.remove(); //remove parent element which is the <li> tag with the "remove" method
} 
});

const filterTodos = (term) => {  //the "term" parameter is gonna be what the user types in
...
//keyup event
search.addEventListener('keyup', () => { //adding a eventListener with 'keyup' to detect when a button is pressed and released
